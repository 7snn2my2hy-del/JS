...
function renderHeroBento() {
  ...
  // RESTURLAUB: laufendes und kommendes Jahr
  {
    ...
    const zeile = a => {
      const pct = Math.max(0, Math.min(100, (a.verplant / URLAUB_ANSPRUCH) * 100));
      return `<div class="ru-mini">
        <div class="ru-mini-top"><span>${a.jahr}</span><span style="color:${farbe(a.rest)}">${zahl(a.rest)} / ${URLAUB_ANSPRUCH}</span></div>
        <div class="bento-progress"><span class="bento-progress-fill" style="width:${pct}%;background:${farbe(a.rest)}"></span></div>
      </div>`;
    };
    tiles.push(`<div class="bento-tile" onclick="openDetail('urlaub')">
      <div class="bento-head"><span class="bento-title">Resturlaub</span></div>
      <div class="bento-primary" style="color:${farbe(a0.rest)}">${zahl(a0.rest)}<span class="bento-unit">Tage</span></div>
      <div class="bento-caption">von ${URLAUB_ANSPRUCH} in ${j0}</div>
...
function tripList(items) {
  ...
  const rows = items.map(u => {
    const tage = Number(u.days) || 0;
    const tageTxt = tage ? ` · ${Number.isInteger(tage) ? tage : String(tage).replace('.', ',')} ${tage === 1 ? 'Tag' : 'Tage'}` : '';
    return `<div class="uy-trip"><span class="uy-trip-name">${esc(u.name)} <span class="uy-trip-mon">| ${MONTH_FULL[tripMonthNum(u)-1]}</span></span><span class="uy-trip-meta">${fmt(tripCost(u))}${tageTxt}</span></div>`;
  }).join('');
...
function resturlaubJahr(j){
  const reisen = urlaube
    .filter(u => parseInt(finTripYear(u), 10) === j)
    .map(u => ({ u, tage: Number(u.days) || 0 }))
    .filter(x => x.tage > 0)
    .sort((a, b) => tripFrom(a.u).localeCompare(tripFrom(b.u)));
  const verplant = reisen.reduce((s, x) => s + x.tage, 0);
  return { jahr: j, reisen, verplant, rest: URLAUB_ANSPRUCH - verplant };
}
