...
function urlaubKontoVerlauf(startSaldo) {
  ...
  const letzte = Object.keys(buckets).map(Number)
    .concat(Object.keys(urlaubKontoOverrides).map(Number).filter(n => !isNaN(n)));
...
function startInlineKontoSaldo(ym, el) {
  ...
  inp.value = typeof alt === 'number' ? fmt(alt) : '';
  inp.placeholder = 'automatisch';
...
function renderUrlaubeDash() {
  ...
  if (verlauf.length) {
    ...
    html += `<div class="uy-konto-hint">Der Verlauf startet mit dem Folgemonat, ausgehend vom Kontostand oben. Kontostand und jeder Monats-Endsaldo lassen sich antippen und überschreiben (• = manuell gesetzt); ab dort rechnet die Automatik wieder weiter. Leeres Feld setzt einen Monat zurück auf automatisch.</div>`;
  }
