  ...
  if (data.a.length) {
    ...
    if (m > 0 || e > 0) {
      ...
      tiles.push(`<div class="bento-tile" onclick="openDetail('a')">
        ...
        <div class="bento-list" style="margin-top:14px">${rows.join('')}</div>
...
function urlaubVorstreckung(jahr) {
  ...
  urlaube.forEach(u => {
    ...
    // Was der Anzahlungsplan nicht abdeckt, wird im Reisemonat faellig
    const rest = kosten - gedeckt;
    if (rest > 0.005 && parseInt(tripRealYear(u), 10) === jahr) summe += rest;
  });
  return summe;
}
...
function urlaubKontoVerlauf(startSaldo) {
  ...
  urlaube.forEach(u => {
    const jahr = parseInt(tripRealYear(u), 10);
    const monat = tripMonthNum(u);
    const kosten = tripCost(u);
    let gedeckt = 0;
    if (Array.isArray(u.payments) && u.payments.length) {
      u.payments.forEach(p => {
    ...
    // Was die Anzahlungen nicht abdecken, wird im Reisemonat faellig - genau wie es der
    // Hinweis im Formular ankuendigt. Ohne das blieb ein Teil der Kosten unberuecksichtigt.
    const rest = kosten - gedeckt;
    if (jahr && monat && rest > 0.005) {
      const ym = jahr * 12 + (monat - 1);
...
function renderUrlaubeDash() {
  ...
  if (verlauf.length) {
    ...
    verlauf.forEach((z, i) => {
      ...
      /* Abschluss je Jahr: Soll gegen Prognose. Das Soll ist normalerweise 0 - das Konto
         soll zum Jahreswechsel ausgeglichen sein. Es darf aber genau um den Betrag im
         Minus stehen, der in diesem Jahr fuer Reisen des Folgejahres vorgestreckt wurde. */
      ...
      if (!naechste || naechste.jahr !== z.jahr) {
        ...
        const abweichung = z.saldo - soll;
        const passt = abweichung >= -0.005;
        html += `<div class="kv-yearend">
            <div class="kv-ye-head"><span>Jahresende ${z.jahr}</span><b class="${passt ? 'ok' : 'negative'}">${passt ? 'im Rahmen' : fmt(Math.abs(abweichung)) + ' zu wenig'}</b></div>
            <div class="kv-move"><span class="kv-lbl">Soll${soll < 0 ? ' (vorgestreckt für ' + (z.jahr + 1) + ')' : ''}</span><span class="kv-ye-val">${fmt(soll)}</span></div>
            <div class="kv-move"><span class="kv-lbl">Prognose</span><span class="kv-ye-val${z.saldo < 0 ? ' negative' : ''}">${fmt(z.saldo)}</span></div>
          </div>`;
      }
    });
  }
