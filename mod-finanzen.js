  ...
  // automatisch aus "Ausgaben nach Konto" abgeleitet, keine doppelte Datenpflege.
  {
    ...
    if (uebNamen.length) {
      ...
      tiles.push(`<div class="bento-tile" onclick="openDetail('uebersicht')">
        ...
        <div class="bento-primary">${fmt(uebSumme)}<span class="bento-unit">/Mt</span></div>
        <div class="bento-caption">${uebNamen.length} ${uebNamen.length === 1 ? 'Überweisung' : 'Überweisungen'}</div>
...
function urlaubKontoVerlauf(startSaldo) {
  ...
  const letzte = Object.keys(buckets).map(Number);
  if (!letzte.length) return [];
  const ymEnd = Math.max(...letzte);
  ...
  for (let ym = ymNow; ym <= ymEnd; ym++) {
    ...
    const zu = (rate > 0 ? [{ label: 'Sparrate', amount: rate }] : []).concat(b.zu);
    const summeZu = zu.reduce((s,x) => s + x.amount, 0);
    const summeAb = b.ab.reduce((s,x) => s + x.amount, 0);
    saldo += summeZu - summeAb;
    zeilen.push({ ym, jahr: Math.floor(ym/12), monat: (ym % 12) + 1, zu, ab: b.ab, saldo });
  }
  return zeilen;
}
  ...
  if (verlauf.length) {
    ...
    verlauf.forEach(z => {
      ...
      html += `${jahrLabel}<div class="kv-month">
          <div class="kv-head"><span class="kv-mon">${MONTH_FULL[z.monat-1]}</span><span class="kv-saldo${z.saldo < 0 ? ' negative' : ''}">${fmt(z.saldo)}</span></div>
          ${bewegungen}
        </div>`;
    });
    html += `<div class="uy-konto-hint">Startpunkt ist der heutige Kontostand; vergangene Monate werden nicht rekonstruiert. Der Saldo rechts ist der Kontostand am Monatsende.</div>`;
  }
