...
function openIncomeMeta() {
  ...
  ['im-target','im-gross-y','im-bonus','im-pension','im-stocks'].forEach(id => bindMoneyInput($(id)));
  ...
  // automatisch aus "Ausgaben nach Konto" abgeleitet, keine doppelte Datenpflege.
  {
    ...
    if (uebNamen.length) {
      ...
      tiles.push(`<div class="bento-tile" onclick="openDetail('uebersicht')">
        ...
        <div class="bento-caption">Am Monatsanfang fällig</div>
...
function saveUrlaub() {
  ...
  if (editUrlaubId) {
    ...
    const { payments: _old, month: _m, daysByYear: _dby, ...rest } = urlaube[i];
    urlaube[i] = { ...rest, ...obj };
  } else {
