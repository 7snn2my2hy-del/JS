...
function renderUrlaubeDash() {
  ...
  const rate = urlaubMonthlyRate();
  const offen = urlaubOffeneZahlungen();
  const kontoNachZahlungen = konto - offen;
  let html = `<div class="uy-konto-head"><span>Kontostand</span><b>${fmt(konto)}</b><span class="saverate-eur">${rate > 0 ? fmt(rate) + ' / Monat Sparrate' : 'Keine Sparrate hinterlegt'}</span></div>`;
  if (offen > 0) {
    html += `<div class="uy-konto-sub"><span>Noch offene geplante Zahlungen</span><b>${fmt(offen)}</b></div>`;
    html += `<div class="uy-konto-sub${kontoNachZahlungen < 0 ? ' negative' : ''}"><span>Kontostand danach</span><b>${fmt(kontoNachZahlungen)}</b></div>`;
  }
  html += `<div class="uy-konto-hint">Der Kontostand zeigt dein reales Guthaben auf dem Sparkonto (Ist). Die Jahreskarten darunter zeigen die Budget-Planung – wie viel dir laut Sparrate pro Jahr zur Verfügung steht und wie viel davon schon für Reisen verplant ist (Soll).</div>`;
...
const FIN_KEYS = {
  ...
  urlaubManualDays:'urlaub_manual_days_v1',
