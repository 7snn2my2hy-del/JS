...
function renderUrlaubeDash() {
  ...
  if (yearlyBudget > 0 && numYears.length) {
    ...
    numYears.forEach(y => {
      ...
      // Invertierte Logik: der Balken zeigt das UEBRIGE Budget in Gruen. Voll = nichts verplant,
      // leer = Budget aufgebraucht. Ist es ueberschritten, wird der Fehlbetrag rot dargestellt,
      // damit ein knappes Minus sichtbar anders aussieht als ein grosses.
      const ueber = balance < 0;
      const fillPct = yearlyBudget > 0
        ? Math.max(0, Math.min(100, Math.round((ueber ? Math.abs(balance) : balance) / yearlyBudget * 100)))
        : 0;
      ...
      html += `<div class="urlaub-year-head${numYears.indexOf(y) === 0 ? ' first-year' : ''}">
          ...
          <div class="uy-bar"><div class="uy-bar-fill ${ueber ? 'over' : ''}" style="width:${fillPct}%"></div></div>
          ${metaGrid}
          ${tripList(items)}</div>`;
    });
  } else {
