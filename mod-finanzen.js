...
function entrySliderHTML(mainHtml, rightHtml, onTap) {
  const tap = onTap ? ` onclick="${onTap}"` : '';
  return swipeInnerHTML(`<div class="entry glass"${tap}>
...
function renderSection(sec) {
  ...
  [...entries]
    ...
    div.className = 'tile-wrap entry-wrap';
    div.dataset.sec = sec;
    div.dataset.id = e.id;
...
/* ================= URLAUBSTAGE =================
   Gesetzliche Feiertage in Bayern. Mariae Himmelfahrt (15.8.) gilt nur in
   ueberwiegend katholischen Gemeinden - Erlangen gehoert nicht dazu. */
...
function _tagPlus(d, n){ const x = new Date(d); x.setDate(x.getDate() + n); return x; }
...
function feiertageBayern(jahr){
  if(_feiertagCache[jahr]) return _feiertagCache[jahr];
  const o = osterSonntag(jahr);
  const liste = {
    [`${jahr}-01-01`]: 'Neujahr',
    [`${jahr}-01-06`]: 'Heilige Drei Könige',
    [_iso(_tagPlus(o, -2))]: 'Karfreitag',
    [_iso(_tagPlus(o,  1))]: 'Ostermontag',
    [`${jahr}-05-01`]: 'Tag der Arbeit',
    [_iso(_tagPlus(o, 39))]: 'Christi Himmelfahrt',
    [_iso(_tagPlus(o, 50))]: 'Pfingstmontag',
    [_iso(_tagPlus(o, 60))]: 'Fronleichnam',
    [`${jahr}-10-03`]: 'Tag der Deutschen Einheit',
    [`${jahr}-11-01`]: 'Allerheiligen',
    [`${jahr}-12-25`]: '1. Weihnachtstag',
    [`${jahr}-12-26`]: '2. Weihnachtstag'
  };
  _feiertagCache[jahr] = liste;
  return liste;
}
function istFeiertag(d){ return !!feiertageBayern(d.getFullYear())[_iso(d)]; }
function istWerktag(d){ const t = d.getDay(); return t >= 1 && t <= 5 && !istFeiertag(d); }

/* Heiligabend und Silvester gelten als halber Arbeitstag und kosten
   entsprechend nur einen halben Urlaubstag. */
const HALBE_TAGE = { '12-24': 'Heiligabend', '12-31': 'Silvester' };
function halberTag(d){ return HALBE_TAGE[_iso(d).slice(5)] || null; }
/* Gewicht eines Tages in Urlaubstagen: 0, 0,5 oder 1 */
function tagesGewicht(d){
  if(!istWerktag(d)) return 0;
  return halberTag(d) ? 0.5 : 1;
}
/* Tageszahl deutsch: 10 Tage, 9,5 Tage, 1 Tag */
function fmtTage(n){
  const txt = Number.isInteger(n) ? String(n) : n.toFixed(1).replace('.', ',');
  return `${txt} ${n === 1 ? 'Tag' : 'Tage'}`;
}

/* Verbrauchte Urlaubstage einer Reise: Werktage Mo-Fr ohne Feiertage.
   Zaehlt nur die Tage, die in das angefragte Jahr fallen. */
function urlaubstage(vonISO, bisISO, jahr){
  if(!vonISO || !bisISO) return 0;
  const von = new Date(vonISO + 'T00:00:00'), bis = new Date(bisISO + 'T00:00:00');
  if(isNaN(von) || isNaN(bis) || bis < von) return 0;
  let n = 0;
...
/* Live-Hinweis im Formular, wie viele Urlaubstage die Reise kostet. */
function updateUrlaubDays(){
  const el = $('urlaub-days-hint');
  if(!el) return;
  const v = deToISO($('urlaub-from').value.trim()), b = deToISO($('urlaub-to').value.trim()) || v;
  if(!v || !b || b < v){ el.textContent = ''; return; }
  const n = urlaubstage(v, b);
  const feier = [], halbe = [];
  for(let d = new Date(v + 'T00:00:00'); d <= new Date(b + 'T00:00:00'); d.setDate(d.getDate()+1)){
    if(d.getDay() < 1 || d.getDay() > 5) continue;
    const name = feiertageBayern(d.getFullYear())[_iso(d)];
    if(name) feier.push(name);
    else if(halberTag(d)) halbe.push(halberTag(d));
  }
  const teile = [];
  if(feier.length) teile.push(`${feier.join(', ')} ${feier.length === 1 ? 'fällt' : 'fallen'} nicht ins Gewicht`);
  if(halbe.length) teile.push(`${halbe.join(', ')} ${halbe.length === 1 ? 'zählt' : 'zählen'} halb`);
  el.textContent = fmtTage(n).replace(/^(\S+) Tage?$/, (m, z) => `${z} ${n === 1 ? 'Urlaubstag' : 'Urlaubstage'}`)
...
/* Resturlaub fuer das laufende und das kommende Jahr.
   30 Tage Anspruch, Werktage Mo-Fr, bayerische Feiertage zaehlen nicht mit. */
function renderResturlaub(){
  const el = $('resturlaub');
  if(!el) return;
  const heute = new Date();
  const jahre = [heute.getFullYear(), heute.getFullYear() + 1];

  el.innerHTML = jahre.map(j => {
    const reisen = urlaube
      .map(u => ({ u, tage: urlaubstage(tripFrom(u), tripTo(u), j) }))
      .filter(x => x.tage > 0)
      .sort((a, b) => tripFrom(a.u).localeCompare(tripFrom(b.u)));
    const verplant = reisen.reduce((s, x) => s + x.tage, 0);
    const rest = URLAUB_ANSPRUCH - verplant;
    const anteil = Math.max(0, Math.min(100, (verplant / URLAUB_ANSPRUCH) * 100));
    const farbe = rest < 0 ? 'var(--danger)' : rest <= 5 ? 'var(--orange)' : 'var(--green)';
    const liste = reisen.length
      ? `<div class="ru-trips">${reisen.map(x =>
          `<div class="ru-trip"><span>${esc(x.u.name)}</span><span>${fmtTage(x.tage)}</span></div>`).join('')}</div>`
      : '<div class="ru-trips"><div class="ru-trip ru-leer">Noch nichts verplant</div></div>';
    return `<div class="ru-jahr">
      <div class="ru-head"><span class="ru-year">${j}</span><span class="ru-rest" style="color:${farbe}">${Number.isInteger(rest) ? rest : rest.toFixed(1).replace('.', ',')} von ${URLAUB_ANSPRUCH} übrig</span></div>
      <div class="ru-bar"><span style="width:${anteil}%;background:${farbe}"></span></div>
      ${liste}
    </div>`;
  }).join('<div class="ru-sep"></div>');
}
...
function renderUrlaubeAll() {
  ...
  renderResturlaub(); renderUrlaubeDash(); renderUrlaube(); renderDeposits();
}
...
function saveUrlaub() {
  ...
  const obj = { name, year, month, cost, from, to };
